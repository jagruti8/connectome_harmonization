...
#SBATCH --job-name ae2dprediction      # Job name
#SBATCH --account rad
#SBATCH --partition rad
#SBATCH --array=0      # Replace with the number of lines in your parameter file - 1
#SBATCH --cpus-per-task=8
#SBATCH --mem=80G
#SBATCH --gres=gpu:rtx2080:1  #(The name of the GPU)
##SBATCH --nodelist=gpunode01
#SBATCH --time=1-23:59:00       #(MAXIMAL TIME OF EXECUTION) hrs:min:sec
#SBATCH --chdir=/data/hagmann_group/harmonization/graph_harmonization_final/batch_scripts/log_files/AUTOENCODER2D/prediction/ #(CHANGE DIRECTORY TO THIS ONE)
...
PARAMS=$(sed -n "$((SLURM_ARRAY_TASK_ID+7))p" $PARAM_FILE)
...
EXTRA_PARAMS="--maindir $MAINDIR --train_set $trainSet --val_set $valSet --test_set $testSet --best 124"
...
python /data/hagmann_group/harmonization/graph_harmonization_final/src/image_image_translation_data_augmentation_autoencoder_2d_prediction.py $ALL_PARAMS # (PATH to the python file you want to execute)
