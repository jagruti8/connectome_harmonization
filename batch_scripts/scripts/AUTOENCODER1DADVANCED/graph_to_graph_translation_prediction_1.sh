...
#SBATCH --job-name autoencoder1Dadv     # Job name
#SBATCH --account rad
#SBATCH --partition rad
#SBATCH --array=0-1      # Replace with the number of lines in your parameter file - 1
#SBATCH --cpus-per-task=8
#SBATCH --mem=120G
#SBATCH --gres=gpu:rtxa6000:1  #(The name of the GPU)
##SBATCH --nodelist=gpunode02
#SBATCH --time=1-23:59:00       #(MAXIMAL TIME OF EXECUTION) hrs:min:sec
#SBATCH --chdir=/data/hagmann_group/harmonization/graph_harmonization_final/batch_scripts/log_files/AUTOENCODER1DADVANCED/prediction/ #(CHANGE DIRECTORY TO THIS ONE)
...
EXTRA_PARAMS="--maindir $MAINDIR --train_set $trainSet --val_set $valSet --test_set $testSet --best latest"
...
python /data/hagmann_group/harmonization/graph_harmonization_final/src/vector_vector_translation_data_augmentation_advanced_2_prediction.py $ALL_PARAMS # (PATH to the python file you want to execute)
