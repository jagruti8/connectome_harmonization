...
print("evaluation_linear_mixed_travelling_harmonization_linear.py starting")
...
from utils.data import read_file_numpy
...
def run_prediction(config):
    ...
    retrieve_path = os.path.join(saved_matrices_path)
    print(retrieve_path)
...
def create_dataset(set1, retrieve_path, behavioral_all_dataset):
    ...
    input_vectors = read_file_numpy(retrieve_path, f"{set1}_input_images.npy")
    ...
    print(f"Predicted vectors shape : {predicted_vectors.shape}")
    print(f"Label site shape : {label_site.shape}")
    ...
    for i, subject in enumerate(subject_IDs):
        ...
        vectors_number_of_fibers_all.append(input_vectors[i])
    ...
    for i, subject in enumerate(subject_IDs):
        track_subjects_IDs.append(subject)
        vectors_number_of_fibers_all.append(predicted_vectors[i])
        y_label_age = behavioral_all_dataset.loc[subject]['Age_in_Yrs']
        y_label_gender = behavioral_all_dataset.loc[subject]['Gender_Numeric']
        y_label_site_all.append(label_site[i])
        y_label_age_all.append(y_label_age)
