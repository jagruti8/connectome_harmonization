...
import numpy as np
import torch
from image_image_translation_data_augmentation_autoencoder_2d_prediction_all import run_prediction

print("image_image_translation_data_augmentation_autoencoder_2d_prediction_main.py starting")
...
if __name__ == "__main__":
    ...
    parameter_combination_file_numbers = np.arange(1,3)
    parameter_combination_file_names = [f"parameters_combinations_{i}.txt" for i in parameter_combination_file_numbers]
    ...
    for file_name_x in parameter_combination_file_names:
        ...
        for j, args_params_x in enumerate(args_params):
            ...
            present_flag, save_path = run_prediction(config, device)
            if present_flag:
                model_all_present += 1
            else:
                model_not_present.append(save_path)
