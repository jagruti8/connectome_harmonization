...
print("vector_vector_translation_data_augmentation_advanced_2.py starting")
...
from utils.data import load_graphs_dict, graph_matrix_vector, save_file, vector_to_matrix_tensor
...
pearson_correlation, combine_vectors, combine_vectors_equal, combine_vectors_linear, matrix_to_graph, nodal_strength_loss, \
kl_divergence_nodal_strength, eigenvalue_difference_batch, find_matrix_shape_from_upper_triangular
...
def main():
    ...
    model_directory_parameters = ["hidden_layer_size", "bottleneck_size", "normalization_l_encoder", "activation_l_encoder", "dropout_encoder", "dropout_rate_encoder",
                                  "normalization_l_domain", "activation_l_domain", "alpha_param", "dropout", "dropout_rate",
                                  ...
                                  "normalization_l1_decoder", "activation_l1_decoder", "dropout_decoder1", "dropout_rate_decoder1",
    ...
    n_inputs, n_outputs = train_dataset.input_tensors[0].shape[0], config.output
    ...
    model = VectorToVectorTranslationAE(input_size=n_inputs, hidden_size=config.hidden_layer_size, bottleneck_size=config.bottleneck_size,
                                        mapping_input_size = config.conditional_vector_size, mapping_hidden_size = config.hidden_layer_size_mapping_network,
                                        mapping_latent_size = config.latent_vector_size, num_layers_latent=config.num_layers_mapping_network_latent,
                                        output_size=n_outputs,
                                        model_encoder=config.model_encoder, normalization_l_encoder=config.normalization_l_encoder,
                                        ...
                                        normalization_l_mapping=config.normalization_l_mapping, activation_l_mapping=config.activation_l_mapping,
                                        model_decoder=config.model_decoder, normalization_l1_decoder=config.normalization_l1_decoder,
                                        activation_l1_decoder=config.activation_l1_decoder, p_decoder1=dp_rate_decoder1).to(device)
    ...
    ### TRAIN THE MODEL
    epoch_list, mae_list, accuracy_list, entropy_list, results = train(model, loss_task, loss_site, cost_matrix, device, optimizer, scheduler, max_epochs,
...
def train(model, loss_task, loss_site, cost_matrix, device, optimizer, scheduler, max_epochs, train_dataloader, val_dataloader, config, save_path):
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for i, train_batch in enumerate(train_dataloader):
            ...
            _, _, site_output, x_hat_2 = model(x, alpha, y3)
            ...
            if config.correlation:
                ...
                for j in range(y2_target.shape[0]):
                    correlation_loss.append(torch.abs(1-pearson_correlation(y2_target[j], x_hat[j])))
                ...
                for j in range(y2_target.shape[0]):
                    for k in range(j + 1, y2_target.shape[0]):
                        siamese_loss.append(torch.abs(
                            pearson_correlation(y2_target[j], y2_target[k])
                            - pearson_correlation(x_hat[j], x_hat[k])))
            ...
            if nroi is None:
                nroi = find_matrix_shape_from_upper_triangular(y2_target[0])
                print(f"No of nodes is : {nroi}")

            if config.nodStr or config.klDiv or config.eigenValue:
                y2_target_matrix = vector_to_matrix_tensor(y2_target, nroi, device)
                x_hat_matrix = vector_to_matrix_tensor(x_hat, nroi, device)

            if config.nodStr:
                nodal_strength_loss_x = nodal_strength_loss(y2_target_matrix, x_hat_matrix)
                loss += (config.nodStr_param * nodal_strength_loss_x)

            if config.klDiv:
                kl_divergence_loss_x = kl_divergence_nodal_strength(y2_target_matrix, x_hat_matrix)
                loss += (config.klDiv_param * kl_divergence_loss_x)

            if config.eigenValue:
                eigenvalue_loss_x = eigenvalue_difference_batch(y2_target_matrix, x_hat_matrix)
        ...
        if ((epoch + 1) % 5 == 0):
            ...
            mae, accuracy, entropy, mae_all, accuracy_all_site = evaluate(model, device, val_dataloader, config, save_path)
            ...
            if accuracy_all_site[0].mean()>=0.5:
...
def create_dataset(set1, set2, path, config):
    ...
    for j, bval in enumerate(bvalues):
        for k, res in enumerate(resolutions_paths):
            ...
            _, vectors_number_of_fibers_input = graph_matrix_vector(subject_IDs_valid_common, graphs_dict_input,
            ...
            if config.augment == True and config.mixed == True and set2 == "train":
                ...
                print(
                    f"Inside the loop for augmented vectors, number of subjects from which pairs are formed : {len(subject_IDs_valid_common)}")
                ...
                for subject1, subject2 in pairs:
                    ...
                    combined_vector_copy1 = combined_vector.copy()

                    combined_vector_copy2 = combined_vector.copy()

                    x = torch.tensor(combined_vector_copy1, dtype=torch.float)
                    y1 = torch.tensor(j * 2 + k, dtype=torch.long)
                    y2 = torch.tensor(combined_vector_copy2, dtype=torch.float)
                    if config.mixed == True and set2 == "train":
                        y3 = conditional_variable_dict[y1.item()]
                    else:
                        y3 = conditional_variable_dict[3]
                    ...
                    augmented_vectors.append(combined_vector_copy2)

                augmented_vectors = np.stack(augmented_vectors)
                np.save(os.path.join(path_subjects, str(bval) + "_" + res + "_augmented_vectors.npy"), augmented_vectors)
    ...
    if config.scaler_input == 3:
        if set2 == "train":
            mean_input = input_tensors_copy.mean(0, keepdim=True)
            std_input = input_tensors_copy.std(0, correction=0, keepdim=True)
            std_input = torch.where(std_input == 0, 1, std_input)
            input_tensors_copy = input_tensors_copy - mean_input
            input_tensors_copy = input_tensors_copy / std_input
            np.save(os.path.join(path, "mean_input.npy"), mean_input.numpy())
            np.save(os.path.join(path, "std_input.npy"), std_input.numpy())
        else:
    ...
    if config.scaler_output == 3 and set2 == "train":
        mean_target = target_tensors_copy.mean(0, keepdim=True)
        std_target = target_tensors_copy.std(0, correction=0, keepdim=True)
        std_target = torch.where(std_target == 0, 1, std_target)
        target_tensors_copy = target_tensors_copy - mean_target
        target_tensors_copy = target_tensors_copy / std_target
        np.save(os.path.join(path, "mean_target.npy"), mean_target.numpy())
        np.save(os.path.join(path, "std_target.npy"), std_target.numpy())

    vector_dataset = VectorDataset(input_tensors_copy, site_outputs, target_tensors_copy, conditional_tensors)

    return vector_dataset

def evaluate(model, device, dataloader, config, path):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            #print("x", torch.equal(x[0], x[1]))
            #print("y1", y1[0], y1[1])
            #print("y2", torch.equal(y2[0], y2[1]))
            #print("y3", y3[0], y3[1])

            _, _, site_output, x_hat_2 = model(x, 1.0, y3)

            #print("x_hat_2", torch.equal(x_hat_2[0], x_hat_2[1]))
            ...
            #print("x_hat", torch.equal(x_hat[0], x_hat[1]))
            #print(x_hat[0])
            #print(x_hat[1])
            ...
            #print("x_hat positive integer", torch.equal(x_hat[0], x_hat[1]))
            ...
            #if i == 0:
                #print("y2_reshape[0]", y2_reshape[0])
                #print("y2_reshape[1]", y2_reshape[1])

            #print(np.array_equal(x_hat[0], x_hat[1]))
            #print(np.array_equal(y2_reshape[0], y2_reshape[1]))
