...
print("evaluation_linear_mixed_travelling_harmonization_graph_metrics.py starting")
...
from utils.data import read_file_numpy, matrix_to_graph_version1, graph_version1_graph_inverse, graph_version1_graph_refined, vector_to_matrix
...
find_matrix_shape_from_upper_triangular_numpy
...
from create_model_path_linear_mixed import create_model_path_function
...
def main():
    ...
    retrieve_path = saved_models_path
    print(retrieve_path)
    ...
    for metric_x in graph_metrics_list:
        ...
        save_path = os.path.join(config.evaluation_set, retrieve_path, "metric", metric_x)
...
def compute_metric(set1, retrieve_path, metric_x):
    ...
    predicted_vectors = read_file_numpy(retrieve_path, f"{set1}_predicted_images.npy")
    label_site = read_file_numpy(retrieve_path, f"{set1}_labels.npy")
    subject_IDs = read_file_numpy(retrieve_path, f"{set1}_subject_IDs.npy")

    predicted_vectors = predicted_vectors.reshape(-1, predicted_vectors.shape[-1])
    label_site = label_site.reshape(-1)
    subject_IDs = subject_IDs.reshape(-1)

    predicted_matrices = []
    nroi = None
    for j, subject_x in enumerate(subject_IDs):
        if nroi is None:
            nroi = find_matrix_shape_from_upper_triangular_numpy(predicted_vectors[j])
            print(f"No of nodes is : {nroi}")
        predicted_matrices.append(vector_to_matrix(predicted_vectors[j], nroi))
    predicted_matrices = np.stack(predicted_matrices)
    print(f"Shape of predicted matrices is : {predicted_matrices.shape}")
